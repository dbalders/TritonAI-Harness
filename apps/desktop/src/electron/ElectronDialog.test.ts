...
describe("ElectronDialog", () => {
  ...
  it.effect("returns false without opening a confirm dialog for an empty message", () =>
    Effect.gen(function* () {
      const dialog = yield* ElectronDialog.ElectronDialog;
      const result = yield* dialog.confirm({ message: "   ", owner: Option.none() });
      assert.isFalse(result);
      assert.equal(showMessageBoxMock.mock.calls.length, 0);
    }).pipe(Effect.provide(ElectronDialog.layer)),
  );

  it.effect("opens a confirmation dialog for the owner window", () =>
    Effect.gen(function* () {
  ...
  it.effect("preserves confirmation request context and cause", () =>
    Effect.gen(function* () {
      ...
      const error = yield* Effect.flip(
        dialog.confirm({ owner: Option.some(owner), message: "  Confirm removal?  " }),
      );
