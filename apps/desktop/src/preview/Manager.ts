...
interface FrameCaptureStartResult {
  readonly added: boolean;
  readonly captureInitialFrame: boolean;
}
...
const makeNativeOperations = Effect.fn("PreviewManager.makeOperations")(function* (
  ...
  const startFrameCapture = Effect.fn("PreviewManager.startFrameCapture")(function* (
        ...
        return Effect.gen(function* () {
          ...
          if (current) {
            if (current.consumers.has(consumer)) {
              return [
                { added: false, captureInitialFrame: false } as FrameCaptureStartResult,
                sessions,
              ] as const;
            }
    ...
    return captureInitialFrame.added;
  });
  ...
  const startRecording = Effect.fn("PreviewManager.startRecording")(function* (tabId: string) {
    ...
    return yield* withTabLifecycleLock(
      tabId,
      Effect.gen(function* () {
        const recordingLeaseCreated = yield* startFrameCapture(tabId, "recording");
        if (!recordingLeaseCreated) return;
        const wc = yield* requireWebContents(tabId);
