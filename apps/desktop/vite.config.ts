...
export default defineConfig({
  run: {
    tasks: {
      build: {
        command:
          "node scripts/assert-electron-keychain-fix.mjs && node scripts/assert-cua-native-asar-fix.mjs && node scripts/build-browser-secret.mjs && node scripts/build-preview-annotation-css.mjs && vp pack",
        dependsOn: ["t3#build"],
        cache: false,
      },
      dev: {
