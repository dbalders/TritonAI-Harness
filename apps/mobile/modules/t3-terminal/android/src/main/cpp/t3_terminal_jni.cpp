...
Java_expo_modules_t3terminal_GhosttyBridge_nativeSnapshot(JNIEnv* env, jclass,
  ...
  while (written_rows < rows &&
    ...
    while (written_cols < cols && ghostty_render_state_row_cells_next(session->row_cells)) {
      ...
      GhosttyBuffer grapheme_buffer{.ptr = nullptr, .cap = 0, .len = 0};
      const auto size_result = ghostty_render_state_row_cells_get(
          session->row_cells, GHOSTTY_RENDER_STATE_ROW_CELLS_DATA_GRAPHEMES_UTF8,
          &grapheme_buffer);
      if (size_result == GHOSTTY_OUT_OF_SPACE && grapheme_buffer.len > 0) {
        utf8.resize(grapheme_buffer.len);
        grapheme_buffer.ptr = utf8.data();
        grapheme_buffer.cap = utf8.size();
        if (ghostty_render_state_row_cells_get(
                session->row_cells, GHOSTTY_RENDER_STATE_ROW_CELLS_DATA_GRAPHEMES_UTF8,
                &grapheme_buffer) == GHOSTTY_SUCCESS) {
          utf8.resize(grapheme_buffer.len);
        } else {
          utf8.clear();
        }
