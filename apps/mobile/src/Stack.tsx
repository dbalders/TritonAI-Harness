...
function workspaceLocationFromState(state: NavigationState) {
  ...
  // A cold deep link may construct a stack containing only the requested
  // overlay. Keep the adaptive workspace anchored to Home until a concrete
  // workspace route exists underneath it.
  if (routes.length === 0) {
    return { pathname: "/", routeKey: undefined };
  }
...
function ThreadReviewRoute(props: ComponentProps<typeof ReviewSheet>) {
  return <ReviewSheet {...props} />;
}
