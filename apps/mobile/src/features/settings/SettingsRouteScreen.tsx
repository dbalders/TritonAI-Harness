...
function ConfiguredSettingsRouteScreen() {
  ...
  const promptSignIn = useCallback(() => {
    Alert.alert(
      `Sign in to ${TRITONAI_CONNECT_NAME}`,
      `Live Activity updates require ${TRITONAI_CONNECT_NAME} so the relay can deliver updates to this device.`,
      [
        { text: "Cancel", style: "cancel" },
        {
          text: "Continue",
  ...
  const linkEnvironments = useCallback(async () => {
    ...
    if (getAgentAwarenessRegistrationStatus() === "registered") {
      ...
      Alert.alert(
        ...
        `This device could not be registered with ${TRITONAI_CONNECT_NAME}, so activity updates won't appear yet. They'll start once registration succeeds.`,
      );
    }
  }, [
    connections,
  ...
  const handleDeviceNotificationsChange = useCallback(
    (enabled: boolean) => {
      ...
      Alert.alert(
        ...
        "Open system Settings to disable notifications for TritonAI Harness.",
        [
          { text: "Cancel", style: "cancel" },
          { text: "Open Settings", onPress: () => void Linking.openSettings() },
        ],
        ...
        <View className="gap-3">
          <SettingsSection title="Account">
            <SettingsRow
              ...
              label="TritonAI Account"
              value={accountLabel}
              onPress={openAccount}
            />
          </SettingsSection>
          <Text className="px-2 text-sm text-foreground-muted">
            TritonAI Harness works locally without signing in. Cloud features are optional.
          </Text>
        </View>
              ...
              onPress={() => {
                void openAndroidLiveUpdateSettings().catch(() => {
                  Alert.alert(
                    ...
                    "Open Android Settings, select TritonAI Harness, then enable Live Updates in Notifications.",
                  );
                });
              }}
            />
