...
export function ThreadRouteScreen(props: ThreadRouteScreenProps) {
  ...
  const environmentId = environmentIdRaw
    ? Option.getOrNull(decodeEnvironmentIdOption(environmentIdRaw))
    : null;
  const threadId = threadIdRaw ? Option.getOrNull(decodeThreadIdOption(threadIdRaw)) : null;
  const routeEnvironmentRuntime = useRemoteEnvironmentRuntime(environmentId);
  const routeConnectionState =
    routeEnvironmentRuntime?.connectionState ?? (environmentId ? "available" : connectionState);
  const routeThreadKey =
    environmentId !== null && threadId !== null ? scopedThreadKey(environmentId, threadId) : null;
...
function ThreadRouteContent(
  ...
  const threadGitControlProps = {
    ...
    showDirectFileControl:
      layout.usesSplitView || (Platform.OS !== "android" && !usesNativeHeaderGlass),
