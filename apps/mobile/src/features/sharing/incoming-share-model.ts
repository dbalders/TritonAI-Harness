...
export async function buildIncomingShareDraft(input: {
  ...
  for (const [index, payload] of input.payloads.entries()) {
    ...
    try {
      const encoded = await input.fileReader.readBase64(uri, PROVIDER_SEND_TURN_MAX_IMAGE_BYTES);
      if (encoded === null || encoded.sizeBytes <= 0) {
        warnings.push(
          `'${resolved?.originalName ?? fallbackName(uri, index, mimeType)}' exceeds the 10 MB attachment limit.`,
        );
        continue;
      }
      const dataUrl = `data:${mimeType};base64,${encoded.base64}`;
      attachments.push({
