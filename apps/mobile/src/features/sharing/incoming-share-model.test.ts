...
describe("incoming native shares", () => {
  it("converts shared text, URLs, and images into a durable composer draft", async () => {
    ...
    const result = await buildIncomingShareDraft({
      ...
      fileReader: {
        readBase64: async () => ({ base64: "YWJj", sizeBytes: 3 }),
        removeOwnedFile,
      },
    });
  ...
  it("skips oversized images and releases the temporary native file", async () => {
    ...
    const readBase64 = vi.fn(async () => ({ base64: "dW51c2Vk", sizeBytes: 6 }));
    const removeOwnedFile = vi.fn(() => Promise.resolve());
  ...
  it("bounds image reads when native metadata does not include a size", async () => {
    const image: SharePayload = {
      shareType: "image",
      value: "content://shared/unknown-size.png",
      mimeType: "image/png",
    };
    const readBase64 = vi.fn(async () => null);

    const result = await buildIncomingShareDraft({
      id: "share-unknown-size",
      createdAt: "2026-07-15T10:00:00.000Z",
      payloads: [image],
      resolvedPayloads: [],
    ...
    expect(result.attachments).toEqual([]);
    expect(result.warnings).toEqual(["'unknown-size.png' exceeds the 10 MB attachment limit."]);
  });
  ...
  it("rejects shared files that exceed the generic attachment limit", async () => {
    ...
    const result = await buildIncomingShareDraft({
      ...
      fileReader: {
        readBase64: async () => ({ base64: "dW51c2Vk", sizeBytes: 6 }),
        persistFile,
        removeOwnedFile,
      },
    });
  ...
  it("releases every temporary file when a share exceeds the attachment limit", async () => {
    ...
    const removeOwnedFile = vi.fn(() => Promise.resolve());
    const readBase64 = vi.fn(async () => ({ base64: "YWJj", sizeBytes: 3 }));
  ...
  it("maps duplicate image payloads to distinct resolved files", async () => {
    ...
    const readBase64 = vi.fn(async (uri: string) =>
      uri.includes("first")
        ? { base64: "Zmlyc3Q=", sizeBytes: 5 }
        : { base64: "c2Vjb25k", sizeBytes: 6 },
    );
    const removeOwnedFile = vi.fn(async () => undefined);
