...
// Both bundle modes feed the archive/staged desktop resource tree in dist.
const emitManagedBuildResources = Effect.fn("emitManagedBuildResources")(function* (
  repoRoot: string,
  serverDir: string,
) {
  const path = yield* Path.Path;
  ...
  yield* fs.writeFileString(
    path.join(serverDir, "dist/plugin-module-loader.cjs"),
    "exports.importPluginModule = (specifier) => import(specifier);\n",
  );
  ...
  yield* fs.remove(path.join(serverDir, "dist/production-integrations"), {
    recursive: true,
    force: true,
  });
  if (pluginSource) {
...
const buildCmd = Command.make(
  ...
  (config) =>
    Effect.gen(function* () {
      ...
      yield* emitManagedBuildResources(repoRoot, serverDir);
    }),
