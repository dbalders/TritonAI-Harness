...
export class SqliteStateExternalDatabaseSqlError extends Schema.TaggedErrorClass<SqliteStateExternalDatabaseSqlError>()(
  "SqliteStateExternalDatabaseSqlError",
  {},
) {
  override get message(): string {
    return "Refusing SQL that can attach, detach, or write another database file.";
  }
}
...
function sqlControlTokens(source: string): ReadonlyArray<string> {
  return (
    source
      .replaceAll(/--[^\r\n]*/g, " ")
      .replaceAll(/\/\*[\s\S]*?\*\//g, " ")
...
function canAccessExternalDatabase(source: string): boolean {
  const tokens = sqlControlTokens(source);
  return (
    tokens.includes("ATTACH") ||
    tokens.includes("DETACH") ||
    (tokens.includes("VACUUM") && tokens.includes("INTO"))
  );
}
...
export const runSqliteState = Effect.fn("runSqliteState")(function* (
  ...
  if (canAccessExternalDatabase(source)) {
    return yield* new SqliteStateExternalDatabaseSqlError();
  }
