...
interface AttachmentSideEffects {
  readonly deletedThreadIds: Map<string, Set<string>>;
  readonly prunedAttachmentIds: Set<string>;
...
const runAttachmentSideEffects = Effect.fn("runAttachmentSideEffects")(function* (
  ...
  protectedAttachmentIds: ReadonlySet<string>,
) {
  ...
  const removeDeletedThreadAttachmentEntry = Effect.fn("removeDeletedThreadAttachmentEntry")(
    function* (threadId: string, protectedAttachmentIds: Set<string>, entry: string) {
      ...
      if (
        !attachmentId ||
        protectedAttachmentIds.has(attachmentId) ||
        !isAttachmentIdOwnedByThread(attachmentId, threadId)
      ) {
        return;
      }
  ...
  const deleteThreadAttachments = Effect.fn("deleteThreadAttachments")(function* (
    ...
    yield* Effect.forEach(
      entries,
      (entry) => removeDeletedThreadAttachmentEntry(threadId, protectedAttachmentIds, entry),
      { concurrency: 1 },
    );
  });

  const removePrunedAttachmentEntry = Effect.fn("removePrunedAttachmentEntry")(function* (
    ...
    if (!attachmentId || !attachmentIds.has(attachmentId)) return;
    yield* fileSystem.remove(path.join(attachmentsRootDir, relativePath), { force: true });
  });

  yield* Effect.forEach(
    sideEffects.deletedThreadIds,
    ([threadId, protectedAttachmentIds]) =>
      deleteThreadAttachments(threadId, protectedAttachmentIds),
    { concurrency: 1 },
  );

  for (const [threadId, retainedPaths] of sideEffects.prunedThreadRelativePaths) {
    const entries = yield* readAttachmentRootEntries;
    for (const entry of entries) {
      const id = parseAttachmentIdFromRelativePath(entry);
      if (
        !id ||
        !sideEffects.prunedAttachmentIds.has(id) ||
        protectedAttachmentIds.has(id) ||
        !isAttachmentIdOwnedByThread(id, threadId) ||
        retainedPaths.has(entry)
      )
        continue;
      const absolutePath = path.join(attachmentsRootDir, entry);
      const info = yield* fileSystem.stat(absolutePath);
      if (info.type === "File") yield* fileSystem.remove(absolutePath, { force: true });
    }
  }
  ...
  function* () {
    ...
    const applyThreadMessagesProjection: ProjectorDefinition["apply"] = Effect.fn(
      ...
      switch (event.type) {
        ...
        case "thread.reverted": {
          ...
          for (const message of existingRows) {
            for (const attachment of message.attachments ?? []) {
              if (
                !(attachment.type === "file" && "path" in attachment) &&
                !liveAttachmentIds.has(attachment.id)
              ) {
                attachmentSideEffects.prunedAttachmentIds.add(attachment.id);
              }
            }
          }
          return;
    ...
    const applyAttachmentSideEffects = Effect.fn("applyAttachmentSideEffects")(
      function* (event: OrchestrationEvent, sideEffects: AttachmentSideEffects) {
        ...
        for (const [threadId, protectedIds] of sideEffects.deletedThreadIds) {
          ...
          if (!recreatedLater) {
            deletedThreadIds.set(threadId, protectedIds);
          }
        }

        // Only prune files referenced by recorded history. Other files may be uploads
        // still waiting for the user to send, and must survive a revert or replay.
        for (const threadId of sideEffects.prunedThreadRelativePaths.keys()) {
          yield* Stream.runForEach(
            eventStore.readAggregateRange({
              aggregateKind: "thread",
            ...
            (historical) =>
              Effect.sync(() => {
                if (
                  historical.aggregateKind !== "thread" ||
                  !sideEffects.prunedThreadRelativePaths.has(historical.aggregateId)
                )
                  return;
                if (historical.type === "thread.message-sent") {
                  for (const attachment of historical.payload.attachments ?? [])
                    if (
                      !(attachment.type === "file" && "path" in attachment) &&
                      isAttachmentIdOwnedByThread(attachment.id, threadId)
                    )
                      sideEffects.prunedAttachmentIds.add(attachment.id);
                } else if (historical.type === "thread.activity-appended") {
                  const answer = decodeQuestionAttachmentAnswer(
                    historical.payload.activity.payload,
                  );
                  if (Option.isSome(answer))
                    for (const attachment of Object.values(
                      answer.value.attachmentsByQuestionId,
                    ).flat())
        ...
        const activityRows = yield* sql<{ attachmentId: string }>`
          SELECT DISTINCT json_extract(attachment.value, '$.id') AS "attachmentId"
          FROM projection_thread_activities AS activities
          JOIN projection_threads AS threads ON threads.thread_id = activities.thread_id
          CROSS JOIN json_each(json_extract(activities.payload_json, '$.attachmentsByQuestionId')) AS question
          CROSS JOIN json_each(question.value) AS attachment
          WHERE threads.deleted_at IS NULL AND activities.kind = 'user-input.answer-submitted'
            AND json_type(attachment.value, '$.id') = 'text'
        `;
        for (const row of activityRows) protectedAttachmentIds.add(row.attachmentId);
        for (const [threadId, protectedIds] of deletedThreadIds) {
          deletedThreadIds.set(threadId, new Set([...protectedIds, ...protectedAttachmentIds]));
        }
        yield* runAttachmentSideEffects(
          {
            deletedThreadIds,
            prunedThreadRelativePaths,
            prunedAttachmentIds: new Set(
              [...sideEffects.prunedAttachmentIds].filter((id) => !protectedAttachmentIds.has(id)),
            ),
          },
    ...
    const runProjectorForEvent = Effect.fn("runProjectorForEvent")(function* (
      ...
      const attachmentSideEffects: AttachmentSideEffects = {
        deletedThreadIds: new Map<string, Set<string>>(),
        prunedAttachmentIds: new Set<string>(),
