  ...
  it.effect("serves client file reads and writes only inside the session roots", () =>
    Effect.gen(function* () {
      ...
      if ((yield* HostProcessPlatform) !== "win32") {
        ...
        yield* fs.writeFileString(target, "unchanged");
        yield* fs.symlink(target, path.join(cwd, "linked.txt"));
        expect(
        ...
        expect(
          (yield* write({
            sessionId: nativeSessionId,
            path: path.join(cwd, "linked-dir", "new", "escape.txt"),
            content: "overwritten",
          }).pipe(Effect.flip))._tag,
        ).toBe("AcpRequestError");
        expect(yield* fs.readFileString(target)).toBe("unchanged");
      }
