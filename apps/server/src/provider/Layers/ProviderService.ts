...
const makeProviderService = Effect.fn("makeProviderService")(function* (
  ...
  const prepareMcpSession = (threadId: ThreadId, providerInstanceId: ProviderInstanceId) =>
    Effect.gen(function* () {
      ...
      capabilities.add("integrations.invoke");
  ...
  const sendTurn: ProviderServiceMethod<"sendTurn"> = Effect.fn("sendTurn")(function* (rawInput) {
    ...
    const fileContext = appendFileAttachmentPrompt({
      prompt: undefined,
      attachments: attachments.filter(
        (attachment): attachment is ChatFileAttachment =>
          attachment.type === "file" &&
          (!("source" in attachment) || attachment.source?._tag !== "pasted-text"),
      ),
      resolvePath: (attachment) =>
        resolveAttachmentPath({ attachmentsDir: serverConfig.attachmentsDir, attachment }),
    });
    if (!appendAttachmentContext(fileContext)) {
      return yield* toValidationError(
        "ProviderService.sendTurn",
