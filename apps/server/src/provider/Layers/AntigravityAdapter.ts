...
const resolveClientFilePath = Effect.fn("AntigravityAdapter.resolveClientFilePath")(
  function* (input: {
    ...
    // Canonicalize the nearest existing ancestor for new files, including the
    // final component for existing files so a symlink cannot escape the root.
    const canonicalPath = (candidate: string): Effect.Effect<string, PlatformError.PlatformError> =>
      input.fileSystem.realPath(candidate).pipe(
        Effect.catchIf(
          (error) => error.reason._tag === "NotFound" && path.dirname(candidate) !== candidate,
          () =>
    ...
    const real = yield* canonicalPath(resolved).pipe(
      Effect.mapError(() =>
        EffectAcpErrors.AcpRequestError.invalidParams(
          `Path '${input.requestPath}' cannot be resolved.`,
        ),
      ),
    );
