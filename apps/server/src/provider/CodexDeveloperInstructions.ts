...
<computer_use_status>Desktop startup status: ${computerUseStatus.label}. ${computerUseStatus.detail} ${computerUseStatus.ready ? "Refresh stale UI references and continue as described above. For other failures, report the actual blocker; direct the user to Settings > General > Computer use only for an actual permission or setup problem." : !computerUseState ? "If asked to use computer use, explain that this environment cannot access the host desktop. Do not direct the user to permission settings for this unsupported environment, silently substitute shell/browser automation, or attempt to grant permissions yourself." : "If asked to use computer use, explain this blocker and direct the user to Settings > General > Computer use. Do not claim the app is missing, silently substitute shell/browser automation, or attempt to grant permissions yourself. Permission changes require restarting Harness before this environment reconnects."}</computer_use_status>
