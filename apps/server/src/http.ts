...
export const assetRouteLayer = HttpRouter.add(
  ...
  Effect.gen(function* () {
    ...
    const disposition = "disposition" in asset ? asset.disposition : undefined;
    const downloadName = "downloadName" in asset ? asset.downloadName : undefined;
    const fileName = "fileName" in asset ? asset.fileName : undefined;
    return yield* HttpServerResponse.file(asset.path, {
      status: 200,
      headers: assetResponseHeaders(
        asset.path,
        disposition !== undefined || asset.mimeType !== undefined
          ? {
