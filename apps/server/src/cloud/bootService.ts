...
/**
 * Installs TritonAI Harness as a per-user boot service. Linux-only for now: systemd
...
export function renderBootServiceUnit(plan: BootServicePlan): string {
  ...
  return [
    "[Unit]",
    "Description=TritonAI Harness server",
...
export class BootServiceInstallError extends Schema.TaggedErrorClass<BootServiceInstallError>()(
  ...
  override get message(): string {
    return "Could not set up the TritonAI Harness background service.";
  }
}
