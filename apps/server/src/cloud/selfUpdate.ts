...
export function resolveServerSelfUpdateCapability(input: {
  ...
  if (input.desktopManaged) return "desktop-managed";
  return input.allowUpstreamT3PackageSelfUpdate === true && input.launcherManaged
    ? "boot-service"
    : null;
}
...
export const make = Effect.fn("cloud.server_self_update.make")(function* (options?: {
  ...
  const capability = resolveServerSelfUpdateCapability({
    ...
    ...(options?.allowUpstreamT3PackageSelfUpdate === undefined
      ? {}
      : { allowUpstreamT3PackageSelfUpdate: options.allowUpstreamT3PackageSelfUpdate }),
  });
