...
/**
 * The update path this process can offer, or null when no in-product update
 * path is available. "desktop-managed" — the TritonAI Harness desktop app
 * spawned this backend and owns its version; only updating the app updates it.
 ...
 *
 * TritonAI Harness does not publish its 0.3.x product versions as the public
 * `t3` npm package. Production callers therefore leave
 * `allowUpstreamT3PackageSelfUpdate` disabled. The opt-in exists only to keep
 * the upstream mechanism testable and easy to rebase; enabling it in a
 * TritonAI build would install the wrong distribution.
 */
export const resolveServerSelfUpdateCapability = Effect.fn(
  ...
  readonly allowUpstreamT3PackageSelfUpdate?: boolean;
}) {
  if (input.desktopManaged) {
    return "desktop-managed" as const;
  }
  if (input.allowUpstreamT3PackageSelfUpdate !== true) {
    return null;
  }
...
export const make = Effect.fn("cloud.server_self_update.make")(function* (options?: {
  ...
  /** Upstream-only test hook. Never enable for TritonAI Harness releases. */
  readonly allowUpstreamT3PackageSelfUpdate?: boolean;
}) {
  ...
  const capability: ServerSelfUpdateCapability | null = yield* resolveServerSelfUpdateCapability({
    ...
    ...(options?.allowUpstreamT3PackageSelfUpdate === true
      ? { allowUpstreamT3PackageSelfUpdate: true }
      : {}),
  });
  ...
  const update: ServerSelfUpdate["Service"]["update"] = Effect.fn(
    ...
    if (capability === "desktop-managed") {
      return yield* failWith(
        "This server is managed by the TritonAI Harness desktop app on its machine; update the desktop app to update it.",
      );
    }
    if (capability === null) {
      return yield* failWith(
        "Automatic server updates are unavailable in this TritonAI Harness distribution.",
      );
    }
