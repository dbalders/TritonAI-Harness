...
it.layer(NodeServices.layer)("resolveServerSelfUpdateCapability", (it) => {
  ...
  it.effect("does not expose public t3 package updates in TritonAI Harness", () =>
    Effect.gen(function* () {
  ...
  it.effect("retains upstream boot-service detection behind an explicit test opt-in", () =>
    Effect.gen(function* () {
      const { home, path } = yield* makeHome();
      const entryPath = path.join(home, ".t3/runtime/versions/0.0.28/node_modules/t3/dist/bin.mjs");
      yield* writeUnitReferencing(home, entryPath);
...
it.layer(NodeServices.layer)("ServerSelfUpdate.update", (it) => {
  ...
  const makeContext = Effect.fn("test.makeContext")(function* (options?: {
    ...
    const service = yield* SelfUpdate.make({
      ...(options?.allowUpstreamT3PackageSelfUpdate === false
        ? {}
        : { allowUpstreamT3PackageSelfUpdate: true }),
      host: {
  ...
  it.effect("keeps the upstream public-package updater dormant by default", () =>
    Effect.gen(function* () {
      const context = yield* makeContext({ allowUpstreamT3PackageSelfUpdate: false });
      const error = yield* context.service.update({ targetVersion: "0.3.2" }).pipe(Effect.flip);
