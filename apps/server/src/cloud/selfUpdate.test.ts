...
const makeHarness = Effect.fn("test.make_self_update_harness")(function* (
  ...
  const selfUpdate = yield* ServerSelfUpdate.make({
    allowUpstreamT3PackageSelfUpdate: options.allowUpstreamT3PackageSelfUpdate ?? true,
  }).pipe(
...
it.layer(NodeServices.layer)("server self update", (it) => {
  ...
  it.effect("does not expose public t3 package updates in TritonAI Harness", () =>
    Effect.gen(function* () {
      const { selfUpdate, order } = yield* makeHarness({
        allowUpstreamT3PackageSelfUpdate: false,
      });
      expect(
        (yield* selfUpdate.update({ targetVersion: "1.1.0" }).pipe(Effect.flip)).reason,
      ).toContain("unavailable in this TritonAI Harness distribution");
