...
const makePairServerConfig = Effect.fn(function* (input: {
  ...
  const authConfig = yield* resolveCliAuthConfig(
    {
      baseDir: Option.some(baseDir),
      devUrl: variant === "dev" ? Option.some(DEV_VARIANT_PLACEHOLDER_URL) : Option.none(),
    },
    Option.some(input.logLevel),
  );
  return ServerConfig.make({
    ...
    cwd: process.cwd(),
    baseDir,
    secretStoreKeys: authConfig.secretStoreKeys,
    legacySecretFingerprints: authConfig.legacySecretFingerprints,
    ...(authConfig.secretStoreKeyFilePath === undefined
      ? {}
      : { secretStoreKeyFilePath: authConfig.secretStoreKeyFilePath }),
    ...derivedPaths,
    staticDir: undefined,
    devUrl,
    devAllowedOrigins: [],
