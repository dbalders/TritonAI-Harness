...
export const TRIAGE_PLAYBOOK = `# TritonAI Harness triage playbook
...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your TritonAI Harness triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `A TritonAI Harness user is \
...
export const buildTriageContext = (input: TriageContextInput) => `# TritonAI Harness triage context
...
- Repo: https://github.com/dbalders/TritonAI-Harness
