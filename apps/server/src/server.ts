...
import * as AnalyticsReactor from "./telemetry/AnalyticsReactor.ts";
...
const ReactorLayerLive = Layer.empty.pipe(
  ...
  Layer.provideMerge(AnalyticsReactor.layer),
...
const IntegrationRegistryStartupLive = IntegrationRegistry.startupLayer.pipe(
  Layer.provide(Layer.mergeAll(ServerSecretStore.layer, ServerSettingsLayerLive)),
);
        ...
        yield* forkParked(
          Effect.gen(function* () {
            ...
            yield* reconcileDesiredCloudLink(`http://127.0.0.1:${address.port}`).pipe(
              ...
              Effect.tap(() =>
                Effect.logInfo("TritonAI Connect desired link reconciled on startup"),
              ),
              Effect.catch((cause) =>
