...
describe("compressImageForStash", () => {
  ...
  it("reports too-large when even the smallest encoding overflows the budget", async () => {
    // Keep the same overflow relationship with bounded synthetic payloads so
    // all codec/quality/scale retries remain deterministic under parallel CI load.
    const tinyBudgetChars = 128;
    const { close } = stubCanvasPipeline(() => 1_024);

    const result = await compressImageForStash(makeFile(1_024), tinyBudgetChars);
