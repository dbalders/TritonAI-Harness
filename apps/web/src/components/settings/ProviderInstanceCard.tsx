...
interface ProviderInstanceCardProps {
  ...
  readonly isManagedByPolicy?: boolean | undefined;
...
export function ProviderInstanceCard({
  ...
  const updateCustomModels = (next: ReadonlyArray<CustomModelDefinition>) => {
    if (isTritonAiManagedInstance) return;
    ...
    return (
      ...
      >
        ...
        <span className="flex h-5 shrink-0 items-center">
          <Switch
            checked={enabled}
            disabled={readOnly || isTritonAiManagedInstance}
  ...
  return (
    <>
      ...
      {isTritonAiManagedInstance ? (
        <SettingsSection title="UCSD managed">
          <p className="px-4 py-3 text-xs text-muted-foreground">
            UC San Diego manages this provider's identity, TritonAI endpoint, Codex runtime paths,
            and model catalog. Personal labels, display preferences, and unrelated environment
            values remain editable.
          </p>
          {tritonAiCredentialRoute !== null && tritonAiCredentialControl ? (
