  ...
  return (
    <>
      ...
      {hasDesktopBridge ? (
        <SettingsRow
          title="Update track"
          description="Stable and Nightly update separately. Install both apps to try Nightly while keeping stable and its data."
          control={
...
export function useSettingsRestore(onRestored?: () => void) {
  ...
  const changedSettingLabels = useMemo(
    () => [
      ...
      ...(appearanceMode !== DEFAULT_THEME_APPEARANCE_MODE ? ["Appearance mode"] : []),
  ...
  const restoreDefaults = useCallback(async () => {
    ...
    const needsThemeReset = previousTheme !== DEFAULT_THEME;
    ...
    const previousAppearanceMode = readAppearanceModePreference(previousTheme);
    const needsAppearanceModeReset = previousAppearanceMode !== DEFAULT_THEME_APPEARANCE_MODE;
    ...
    const rollbackThemeState = () => {
      ...
      if (needsAppearanceModeReset) setAppearanceMode(previousAppearanceMode);
    ...
    if (needsThemeReset && !setTheme(DEFAULT_THEME)) {
    ...
    if (needsAppearanceModeReset && !setAppearanceMode(DEFAULT_THEME_APPEARANCE_MODE)) {
      rollbackThemeState();
      notifyThemeRestoreFailure();
      return;
    }
      ...
      <SettingsSection id="appearance" title="Appearance">
        ...
        {showEnvironmentIdentification ? (
          <SettingsRow
            ...
            description="Show decorative artwork in Nightly and Dev headers. The environment label stays visible when artwork is off."
            resetAction={
            ...
            control={
              <Switch
                aria-label="Show header artwork"
                checked={settings.environmentIdentificationMode === "artwork"}
                onCheckedChange={(checked) =>
                  updateSettings({ environmentIdentificationMode: checked ? "artwork" : "pill" })
                }
              />
            }
          />
