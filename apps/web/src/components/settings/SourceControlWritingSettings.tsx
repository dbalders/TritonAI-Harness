...
import type { ServerProvider, SourceControlWritingStyleMode } from "@t3tools/contracts";
...
export function filterSupportedSourceControlWriterProviders(
  providers: ReadonlyArray<ServerProvider>,
): ServerProvider[] {
  return providers.filter((provider) => getDriverOption(provider.driver) !== undefined);
}
...
export function SourceControlWritingSettingsSection() {
  ...
  const supportedServerProviders = useMemo(
    () => filterSupportedSourceControlWriterProviders(serverProviders),
    [serverProviders],
  );
  ...
  const defaultModelSelection = resolveAppModelSelectionState(settings, supportedServerProviders);
  ...
  const instanceEntries = sortProviderInstanceEntries(
    applyProviderInstanceSettings(
      deriveProviderInstanceEntries(supportedServerProviders),
      settings,
    ),
  );
