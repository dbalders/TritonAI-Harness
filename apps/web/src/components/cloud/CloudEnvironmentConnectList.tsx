...
import { TRITONAI_CONNECT_NAME, type EnvironmentId } from "@t3tools/contracts";
...
/**
 * The user's TritonAI Connect environments from relay discovery, each with a
  ...
  const connectEnvironment = async (environment: RelayClientEnvironmentRecord) => {
    ...
    if (result._tag === "Success") {
      toastManager.add({
        ...
        description: `Connecting to ${environment.label} through ${TRITONAI_CONNECT_NAME}.`,
      });
      return;
    }
    ...
    const message =
      cause instanceof Error
        ? cause.message
        : `Could not connect the ${TRITONAI_CONNECT_NAME} environment.`;
      ...
      return (
        <div className={ITEM_ROW_CLASSNAME}>
          <p className="text-sm font-medium text-destructive">
            Could not load {TRITONAI_CONNECT_NAME} environments
          </p>
