...
import { shouldShowArm64IntelBuildWarning } from "./desktopUpdate.logic";
...
const SidebarProjectThreadList = memo(function SidebarProjectThreadList(
  ...
  const {
    ...
    emptyThreadLabel = "No threads yet",
    projectCwd,
...
const SidebarProjectItem = memo(function SidebarProjectItem(props: SidebarProjectItemProps) {
  ...
  const projectExpanded = forceThreadPanelOpen ? true : storedProjectExpanded;
  ...
  const handleProjectButtonContextMenu = useCallback(
    (event: React.MouseEvent<HTMLButtonElement>) => {
      ...
      void (async () => {
        ...
        actionHandlers.set(pinActionId, () => {
          setProjectsPinned(projectPhysicalKeys, !isProjectPinned);
        });
        const makeLeaf = (
      ...
      {!hideProjectHeader ? (
        <div className="group/project-header relative">
          <SidebarMenuButton
            ...
            size="sm"
            className={`h-8 gap-2 rounded-md px-2 py-1.5 pr-14 text-left hover:bg-sidebar-row-hover group-hover/project-header:bg-sidebar-row-hover group-hover/project-header:text-sidebar-foreground max-sm:pr-20 ${
      ...
      {pinnedProjects.length > 0 ? (
        <SidebarGroup className="px-2 pt-2 pb-1">
          ...
          >
            <SidebarMenu>
              <SortableContext
                items={pinnedProjects.map((project) => project.projectKey)}
...
export default function Sidebar() {
  ...
  const handleProjectDragEnd = useCallback(
    (event: DragEndEvent) => {
      ...
      reorderProjects(
        sortedProjects.flatMap(projectMemberPhysicalKeys),
        projectMemberPhysicalKeys(activeProject),
        projectMemberPhysicalKeys(overProject),
      );
    },
    [reorderProjects, sidebarProjectByKey, sidebarProjectSortOrder, sortedProjects, updateSettings],
  );
  ...
  const visibleSidebarThreadKeys = useMemo(
    ...
    [
      ...
      projectExpandedById,
      routeThreadKey,
      sidebarProjectsForThreadNavigation,
      threadsByProjectKey,
    ],
  );
  ...
  const arm64IntelBuildWarningDescription =
    desktopUpdateState && showArm64IntelBuildWarning
      ? "This Mac has Apple Silicon, but TritonAI Harness is running the Intel build under Rosetta. Run the next full TritonAI Installer update to switch to the native build."
      : null;
