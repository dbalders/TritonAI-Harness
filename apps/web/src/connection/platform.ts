...
function clientMetadata() {
  ...
  return {
    label: desktop ? "TritonAI Harness Desktop" : "TritonAI Harness Web",
    ...
    const cloudSession = CloudSession.of({
      clerkToken: Effect.gen(function* () {
        ...
        if (session === null) {
          return yield* new ConnectionBlockedError({
            reason: "authentication",
            detail: "Sign in to TritonAI Harness Cloud to connect this environment.",
          });
        }
        ...
        if (token === null) {
          return yield* new ConnectionBlockedError({
            reason: "authentication",
            detail: "The TritonAI Harness Cloud session is unavailable.",
          });
        }
        return token;
      }),
