...
export function makeAggregateState(input: {
  ...
  const terminalCandidates = input.terminalState
    ? [
        ...input.activeStates.filter((state) => state.threadId !== input.terminalState?.threadId),
        input.terminalState,
      ]
    : input.activeStates;
  const recentTerminalStates = terminalCandidates
    .filter((state) => isRecentTerminalState(state, input.nowMs))
    .sort((a, b) => b.updatedAt.localeCompare(a.updatedAt));
