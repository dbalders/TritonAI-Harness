...
describe("makeAggregateState", () => {
  ...
  it("uses the just-published terminal state while other agents remain active", () => {
    const finishingThread = {
      ...state,
      threadId: "thread-finishing" as RelayAgentActivityState["threadId"],
      updatedAt: "1970-01-01T00:58:00.000Z",
    };
    const otherActive = {
      ...state,
      threadId: "thread-active" as RelayAgentActivityState["threadId"],
      updatedAt: "1970-01-01T00:57:00.000Z",
    };
    const terminalState: RelayAgentActivityState = {
      ...finishingThread,
      phase: "completed",
    ...
    const aggregate = AgentActivityPublisher.makeAggregateState({
      activeStates: [finishingThread, otherActive],
      terminalState,
      nowMs: hourMs,
    });
    ...
    expect(aggregate?.activities).toMatchObject([
      { threadId: "thread-active", phase: "running" },
      { threadId: "thread-finishing", phase: "completed" },
    ]);
  });
