 ...
 */
// @effect-diagnostics nodeBuiltinImport:off - The isolated smoke home needs fresh encryption key material.
...
const encodeSmokeKeyring = Schema.encodeSync(
  Schema.fromJsonString(Schema.Struct({ version: Schema.Literal(1), active: Schema.String })),
);
...
const smokeCliArchive = Effect.fn("smokeCliArchive")(function* (input: {
  ...
  // Harness requires an explicit per-instance key provider. This disposable
  // key never leaves the scratch directory and is removed with the smoke home.
  const keyFile = path.join(scratch, "secret-store-keyring.json");
  yield* fs.writeFileString(
    keyFile,
    encodeSmokeKeyring({ version: 1, active: NodeCrypto.randomBytes(32).toString("base64") }),
    { mode: 0o600 },
  );
    ...
    ChildProcess.make(
      ...
      ["serve", "--host", "127.0.0.1", "--port", String(port), "--no-browser"],
      {
        ...
        env: {
          ...
          TEMP: scratch,
          T3CODE_HOME: home,
          TRITONAI_SECRET_STORE_KEY_FILE: keyFile,
        },
        extendEnv: false,
      },
    ),
