...
function reportLabels(report) {
  ...
  if (report.installStatus === "failed" || report.checkStatus === "failed") {
    labels.push("checks-failed");
  }
...
- Merge base: \`${report.mergeBase}\` (${report.mergeBaseDate}), ${report.upstreamCommitsToMerge} upstream commits to merge
- Result: \`${report.status}\`
- Merge: \`${report.mergeStatus}\`
- Dependencies: \`${report.installStatus}\`
...
function describeAncestry({ upstreamSha, downstreamSha, cwd }) {
  const mergeBase = git(["merge-base", upstreamSha, downstreamSha], { cwd });
  const mergeBaseDate = git(["log", "-1", "--format=%cs", mergeBase], { cwd });
  const upstreamCommitsToMerge = Number(
    git(["rev-list", "--count", `${downstreamSha}..${upstreamSha}`], { cwd }),
  );
  return { mergeBase, mergeBaseDate, upstreamCommitsToMerge };
}

function autoMergePullRequest({ branch, cwd }) {
  const repo = process.env.TRITONAI_SYNC_GITHUB_REPO ?? process.env.GH_REPO;
  const method = resolvePrMergeMethod();
...
function main() {
  ...
  const ancestry = describeAncestry({ upstreamSha, downstreamSha, cwd: repoRoot });
  ...
  const report = {
    ...
    downstreamSha,
    syncBranch: branch,
    mergeBase: ancestry.mergeBase,
    mergeBaseDate: ancestry.mergeBaseDate,
    upstreamCommitsToMerge: ancestry.upstreamCommitsToMerge,
    mergeStatus: "not-run",
  ...
  try {
    ...
    const installResult = run("vp", ["install", "--frozen-lockfile"], {
      cwd: worktree,
      env: checkEnv,
      check: false,
    });
    report.installStatus = installResult.status === 0 ? "passed" : "failed";
    if (installResult.status !== 0) {
      report.summary = "Merge completed, but the frozen dependency install failed.";
      report.risks = [
        "Fix the merged worktree's package manifests or pnpm lockfile before validation.",
      ];
      console.log(JSON.stringify(report, null, 2));
      return 2;
    }
    ...
    const checksOk =
      report.installStatus === "passed" &&
      (report.checkStatus === "passed" || report.checkStatus === "skipped");
