...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    // Precedence (--home-dir > worktree .t3 > ambient home) is resolved
    ...
    const output: NodeJS.ProcessEnv = {
      ...
      [TRITONAI_HOME_ENV]: resolvedBaseDir,
    };
    delete output[LEGACY_T3CODE_HOME_ENV];
...
export function runDevRunnerWithInput(input: DevRunnerCliInput) {
  return Effect.gen(function* () {
    ...
    // outrank ambient TRITONAI_HOME/T3CODE_HOME values. `--home-dir` still wins.
    ...
    const resolvedT3Home =
      ...
      worktreeHome ??
      (hostEnvironment[TRITONAI_HOME_ENV]?.trim() || undefined) ??
      (hostEnvironment[LEGACY_T3CODE_HOME_ENV]?.trim() || undefined);
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  t3Home: Flag.string("home-dir").pipe(
    Flag.withDescription(
      "Explicit TritonAI Harness data directory (equivalent to TRITONAI_HOME). Inside a git worktree this defaults to that worktree's own .t3 so dev state stays off the shared home.",
    ),
    Flag.optional,
