...
import { TRITONAI_APP_BASE_NAME } from "./tritonai.ts";
...
export const T3ProjectFileScript = Schema.Struct({
  name: trimmedNonEmpty({
    description: `Display name for the script, shown in the ${TRITONAI_APP_BASE_NAME} scripts menu.`,
  }),
  command: trimmedNonEmpty({
    description: `Shell command executed in a ${TRITONAI_APP_BASE_NAME} terminal at the project root.`,
  }),
  ...
  description: `A project script that team members can import into ${TRITONAI_APP_BASE_NAME}.`,
});
...
export const T3ProjectFile = Schema.Struct({
  ...
  iconPath: Schema.optionalKey(
    trimmedNonEmpty(
      {
        description: `Workspace-relative path to the project icon (e.g. "assets/logo.svg"). Checked before ${TRITONAI_APP_BASE_NAME}'s built-in icon locations.`,
      },
  ...
  scripts: Schema.optionalKey(
    Schema.Array(T3ProjectFileScript)
      .annotate({
        description: `Project scripts shared with everyone who opens this repository in ${TRITONAI_APP_BASE_NAME}.`,
      })
      .check(Schema.isMaxLength(T3_PROJECT_FILE_MAX_SCRIPTS)),
  ),
}).annotate({
  title: "T3 project file",
  description: `Checked-in project configuration for ${TRITONAI_APP_BASE_NAME} (t3.json at the repository root). See https://t3.codes for documentation.`,
});
