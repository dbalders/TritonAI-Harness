...
2. Choose a base directory that belongs only to the current worktree or test:
   - In a linked worktree, use its ignored `.t3` directory for reusable worktree-local state.
   - Use `mktemp -d /tmp/t3code-test.XXXXXX` for disposable state and retain the printed absolute path.
3. Start the full web stack with `vp run dev`. Add `--share` when the user needs to open it from another tailnet device. In a linked worktree it defaults to that worktree's gitignored `.t3`; outside a linked worktree, always pass `--home-dir <base-dir>` so the test cannot fall back to the shared TritonAI home.
...
Treat a base directory as disposable only when it was created or deliberately selected for the current test. Never delete or directly seed the shared `~/.tritonai-harness` directory or a legacy shared `~/.t3` directory. Prefer starting with a new temporary base directory over clearing state of uncertain ownership.

The worktree-local default deliberately outranks ambient `TRITONAI_HOME` and legacy `T3CODE_HOME` values; do not pass either shared home through to a worktree dev server.
...
The helper refuses to write to the shared `~/.tritonai-harness` or legacy `~/.t3` directory by default and creates a database backup before each mutation.
