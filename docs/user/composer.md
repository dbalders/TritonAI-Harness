...
also send files to TritonAI Harness through another app's system share sheet.
...
TritonAI Harness remembers your provider, model, and model options for new threads. A
...
Transcription runs on your device. TritonAI Harness deletes the temporary audio after
...
Provider commands must start the message to run. TritonAI Harness commands such as
...
complete TritonAI Harness tells you and leaves the chip for you to remove or replace. A chip whose
...
automatically. HTML previews cannot access your TritonAI Harness session.
