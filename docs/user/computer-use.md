...
Computer-use calls appear as distinct activity cards showing the action, session name when available, and progress or failure status. If the screen changes before an action can run, the card shows a neutral **Window changed** step and the agent is instructed to inspect the window again and continue with a fresh reference. This does not indicate a permissions problem. Expand a card to inspect its tool details. The agent is instructed to explain permission failures and use the desktop tools when you request computer use.
