...
TritonAI Harness requires OpenCode 1.14.19 or newer, including when you connect an existing
OpenCode server.

## Local or external server

Leave **Server URL** empty to let TritonAI Harness start OpenCode locally. A password in
provider settings applies to both that server and TritonAI Harness's connection. With no
...
settings. TritonAI Harness uses only that configured password for an external server; it
...
restart before TritonAI Harness can see configuration changes.
