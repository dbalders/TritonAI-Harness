...
5. After Harness finishes, select **View PR** to follow its review.
...
If GitHub is missing or lacks required authorization, Harness can install or enable the bundled plugin and directs you to **Settings → Plugins → GitHub**. Connect your contributor account and enable identity read, repository read/write, and pull-request creation. Then return to **Settings → Skills** and select **Share with UCSD** again. Harness revalidates the current local files and safely resumes an interrupted content-addressed branch or existing pull request rather than overwriting remote content.
