...
The desktop process owns permission onboarding and driver lifetime. It requests macOS Accessibility and Screen Recording after Electron is ready, does not start the driver until both permissions are present, uses standard permission mode, disables driver telemetry and update checks, and stops the child when the application scope closes.
