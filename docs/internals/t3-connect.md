...
TritonAI Harness keeps the upstream Connect protocol but does not offer public-package background
service installation during setup. See [distribution policy](../user/background-service.md).
Stable and Nightly use separate desktop callback schemes; see [desktop identity](./desktop-release-identity.md).
